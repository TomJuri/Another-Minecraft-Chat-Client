...
public class MojangAPI {
    ...
    public static String getUUID(final String username) throws IOException {
        final String js = new String(IOUtils.readFully(new URL(Hosts.MOJANG_APISERVER + "/users/profiles/minecraft/" + username).openStream()));
        if (js.isEmpty()) return null;

        return JsonParser.parseString(js).getAsJsonObject().get("id").getAsString();
    }
     ...
     *                               the user
     */
    public static MojangUser authenticateUser(final String username, final String password, final String endpoint) throws MalformedURLException, IOException {
        ...
        if (json.has("error")) {
            final String errorMsg = json.has("errorMessage") ?
            ...
            return mUser;
        } else throw new IOException(Messages.getString("MojangAPI.mojangResponseError"));
    }
     ...
     */
    public static RequestResponse makeJSONRequest(final String endpoint, final Map<String, JsonElement> values) throws MalformedURLException, IOException {
    ...
    public static String getSkin(final String uuid) throws IOException {
        final JsonObject el = JsonParser.parseString(new String(IOUtils.readFully(new URL(Hosts.MOJANG_SESSIONSERVER + "/session/minecraft/profile/" + uuid).openStream())))
                                        .getAsJsonObject();
